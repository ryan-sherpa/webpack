...
class Pack {
	constructor(logger, maxAge, prevBuildDirectory, currentBuildDirectory) {
		...
		this.logger = logger;
		this.maxAge = maxAge;
		this.prevBuildDir = prevBuildDirectory;
		this.currentBuildDir = currentBuildDirectory;

		console.log('this.prevBuildDir', this.prevBuildDir)
		console.log('this.currentBuildDir', this.currentBuildDir)
	}
	...
	get(identifier, etag) {
		let id = identifier;
		let info = this.itemInfo.get(id);

		if (info === undefined && this.prevBuildDir && this.currentBuildDir) {

			id = identifier.replace(this.currentBuildDir, this.prevBuildDir);
			info = this.itemInfo.get(id);
			if (info !== undefined) { console.log("HIIIIIIIT") };
		}
		...
		if (loc === -1) {
			...
			return this.content[loc].get(id);
		}
	}
...
class PackFileCacheStrategy {
	...
	constructor({
		...
		this.prevBuildDirectory = compiler.options.cache.prevBuildDirectory;
		this.currentBuildDirectory = compiler.options.cache.currentBuildDirectory;
	...
	_openPack() {
		...
		return this.fileSerializer
			...
			.then(pack => {
				...
				return new Pack(logger, this.maxAge, this.prevBuildDirectory, this.currentBuildDirectory);
			})
			.catch(err => {
				this.logger.warn(
